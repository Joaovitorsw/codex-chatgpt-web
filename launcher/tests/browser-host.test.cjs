...
test("off-on-off fresh conversation changes retire completed history before it can be reused", async () => {
  ...
  for (const savedChats of [false, true]) {
    ...
    vm.runInNewContext("let applyingPendingFreshConversation = false;\n" +
      ...
      handle: (registeredChannel, callback) => { if (registeredChannel === channel) handler = callback; },
      browserHost: fixture, releaseRetainedConversation,
